			...
			for (size_t i = 0; i < _varDeclStatement.declarations().size(); ++i)
				if (auto const& decl = _varDeclStatement.declarations()[i])
				{
					if (tupleType->components()[i])
						...
						define(m_context.addLocalVariable(*decl));
				}
		}
		else
		{
...
bool IRGeneratorForStatements::visit(Assignment const& _assignment)
{
	...
	if (*_assignment.annotation().type != *TypeProvider::emptyTuple()) // tuple assignments
...
void IRGeneratorForStatements::endVisit(FunctionCall const& _functionCall)
{
	...
	case FunctionType::Kind::Internal:
	{
		...
		args = IRVariable(_functionCall.expression()).stackComponents() + args;
		define(_functionCall) <<
	...
	case FunctionType::Kind::ArrayPush:
	{
		...
		if (arguments.empty())
		{
			...
			m_code << "let " << slotName << ", " << offsetName << " := " <<
				...
				"(" << IRVariable(memberAccessExpression).name() << ")\n";
...
void IRGeneratorForStatements::endVisit(MemberAccess const& _memberAccess)
{
	...
	case Type::Category::Address:
	{
		...
		else if (set<string>{"send", "transfer"}.count(member))
		{
			...
			// TODO be careful about types. payable address.
			//define(_memberAccess, _memberAccess.expression());
		}
		else if (set<string>{"call", "callcode", "delegatecall", "staticcall"}.count(member))
		{
			// TODO be careful about types. address.
			//define(_memberAccess, _memberAccess.expression());
		}
		else
	...
	case Type::Category::Array:
	{
		...
		else if (member == "pop" || member == "push")
		{
			...
			//m_code << IRVariable(_memberAccess).define(m_context, _memberAccess.expression());
			//m_code << IRVariable(_memberAccess, *_memberAccess.expression().annotation().type).define(m_context, _memberAccess.expression());
		}
		else
...
void IRGeneratorForStatements::endVisit(IndexAccess const& _indexAccess)
{
	...
	if (baseType.category() == Type::Category::Mapping)
	{
		...
		templ("base", IRVariable(_indexAccess.baseExpression()).name());
		if (keyType.sizeOnStack() == 0)
			templ("key", "");
		else
			templ("key", ", " + IRVariable(*_indexAccess.indexExpression()).name());
		m_code << templ.render();
...
void IRGeneratorForStatements::setLValue(Expression const& _expression, IRLValue _lvalue)
{
	...
	define(_expression, fetchCurrentLValue()); // TODO: do we need this always?
